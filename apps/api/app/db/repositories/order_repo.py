...
class OrderRepository:
    ...
    async def get_by_client_key(self, key: str) -> Order | None:
        result = await self.db.execute(select(Order).where(Order.client_order_key == key))
        return result.scalar_one_or_none()

    async def get_by_broker_id(self, broker_id: str) -> Order | None:
        result = await self.db.execute(select(Order).where(Order.broker_order_id == broker_id))
        return result.scalar_one_or_none()
    ...
    async def has_active_for_ticker_side(self, ticker: str, side: str) -> bool:
        result = await self.db.execute(
            select(Order.id)
            .where(
                Order.ticker == ticker,
