...
class LiveReadinessService:
    ...
    async def evaluate(self) -> dict[str, Any]:
        ...
        last_test_at = self._normalize_timestamp(live_connection.last_test_at) if live_connection else None
        broker_test_recent = bool(
            live_connection
            and live_connection.last_test_ok
            and last_test_at
            and now - last_test_at <= _BROKER_TEST_MAX_AGE
        )
        telegram_ready = bool(
        ...
        checks = [
            ...
            self._build_check(
                ...
                passed=demo_validated_at is not None,
                detail=(
                    f"Demo validation recorded by {evidence.get('demo_validated_by', 'an admin')}."
                    if demo_validated_at
                    else "Record that demo/paper trading has been reviewed and accepted."
                ),
            ...
            self._build_check(
                ...
                passed=broker_verified_at is not None,
                detail=(
                    f"Live broker test review recorded by {evidence.get('broker_test_verified_by', 'an admin')}."
                    if broker_verified_at
                    else "Record that the latest live broker connection test was reviewed."
                ),
            ...
            self._build_check(
                ...
                passed=telegram_verified_at is not None,
                detail=(
                    f"Telegram alert review recorded by {evidence.get('telegram_test_verified_by', 'an admin')}."
                    if telegram_verified_at
                    else "Send a Telegram test alert and record that it arrived correctly."
                ),
            ...
            self._build_check(
                ...
                passed=kill_switch_tested_at is not None,
                detail=(
                    f"Kill-switch drill recorded by {evidence.get('kill_switch_tested_by', 'an admin')}."
                    if kill_switch_tested_at
                    else "Perform and record a kill-switch drill before allowing live automation."
                ),
            ...
            self._build_check(
                ...
                passed=app_settings.live_trading_unlocked,
                detail=(
        ...
        ready_for_live = eligible_for_unlock and app_settings.live_trading_unlocked
        blockers = [
            check["detail"]
            for check in checks
        ...
        *,
        ...
        if action == "unlock_live":
            ...
            field_map = {
                ...
                "record_broker_test": ("broker_test_verified_at", "broker_test_verified_by", "broker_test_notes"),
                "record_telegram_test": ("telegram_test_verified_at", "telegram_test_verified_by", "telegram_test_notes"),
                "record_kill_switch_test": ("kill_switch_tested_at", "kill_switch_tested_by", "kill_switch_test_notes"),
            }
            if action == "record_broker_test":
                broker_check = next(
                    check for check in evaluation["checks"] if check["key"] == "live_broker_test_recent"
                )
