...
def _no_channels_ctx():
    ...
    return patch("app.services.alert_service.settings", **{
        "ALERT_EMAIL_TO": "",
        "SMTP_HOST": "",
        "TELEGRAM_BOT_TOKEN": "",
        "TELEGRAM_CHAT_ID": "",
...
class TestEmailChannel:
    ...
    async def test_skipped_when_smtp_host_missing(self, db):
        with patch("app.services.alert_service.settings", **{
...
class TestTelegramChannel:
    async def test_skipped_when_chat_id_empty(self, db):
        ...
        with patch("app.services.alert_service.settings", **{
            ...
            with patch.object(httpx.AsyncClient, "post", new_callable=AsyncMock) as mp:
                svc = AlertService(db)
                await svc.send(alert_type="t", title="T", message="M")
    ...
    async def test_fires_when_configured(self, db):
        ...
        with patch("app.services.alert_service.settings", **{
            ...
            with patch.object(httpx.AsyncClient, "post", new_callable=AsyncMock, return_value=resp) as mp:
                svc = AlertService(db)
                await svc.send(alert_type="t", title="T", message="M", severity="critical")
...
class TestAlertHelpers:
    ...
    async def test_alert_abnormal_slippage_warning(self, db):
        with _no_channels_ctx():
            await alert_abnormal_slippage(
                db, order_id="o1", ticker="TSLA", side="buy",
                expected_price=100.0, fill_price=101.0,
                slippage_pct=1.0, slippage_value=1.0,
            )
        await db.commit()
        a = (await db.execute(select(Alert).where(Alert.alert_type == "abnormal_slippage"))).scalar_one()
        assert a.severity == "warning"  # slippage_pct < 1.5

    async def test_alert_abnormal_slippage_critical(self, db):
        with _no_channels_ctx():
            await alert_abnormal_slippage(
                db, order_id="o2", ticker="TSLA", side="buy",
                expected_price=100.0, fill_price=102.0,
                slippage_pct=2.0, slippage_value=2.0,
            )
        await db.commit()
        a = (await db.execute(select(Alert).where(Alert.alert_type == "abnormal_slippage"))).scalar_one()
    ...
    async def test_alert_daily_loss_breach(self, db):
        ...
        await db.commit()
        a = (await db.execute(select(Alert).where(Alert.alert_type == "daily_loss_breach"))).scalar_one()
        assert a.severity == "critical"
    ...
    async def test_alert_broker_disconnected(self, db):
        ...
        await db.commit()
        a = (await db.execute(select(Alert).where(Alert.alert_type == "broker_disconnected"))).scalar_one()
        assert a.severity == "critical"
    ...
    async def test_alert_trade_submitted_buy(self, db):
        with _no_channels_ctx():
            await alert_trade_submitted(
                db, strategy_name="ORB", ticker="AMD", side="buy",
                qty=10, price=150.0, confidence=0.85, reason="breakout",
            )
        await db.commit()
        a = (await db.execute(select(Alert).where(Alert.alert_type == "trade_submitted"))).scalar_one()
    ...
    async def test_alert_trade_submitted_sell(self, db):
        with _no_channels_ctx():
            await alert_trade_submitted(
                db, strategy_name="ORB", ticker="AMD", side="sell",
                qty=10, price=155.0,
            )
        await db.commit()
        a = (await db.execute(select(Alert).where(Alert.alert_type == "trade_submitted"))).scalar_one()
    ...
    async def test_alert_stop_out(self, db):
        with _no_channels_ctx():
            await alert_stop_out(
                db, strategy_name="ORB", ticker="SPY",
                exit_price=100.0, entry_price=102.0, pnl=-2.0,
            )
        await db.commit()
    ...
    async def test_alert_take_profit_full(self, db):
        with _no_channels_ctx():
            await alert_take_profit(
                db, strategy_name="ORB", ticker="AAPL",
                exit_price=110.0, entry_price=100.0, pnl=10.0,
            )
        await db.commit()
    ...
    async def test_alert_take_profit_partial(self, db):
        with _no_channels_ctx():
            await alert_take_profit(
                db, strategy_name="ORB", ticker="AAPL",
                exit_price=105.0, entry_price=100.0, pnl=5.0,
    ...
    async def test_alert_daily_summary_clean(self, db):
        with _no_channels_ctx():
            await alert_daily_summary(
                db, date_str="2026-04-24", total_trades=10,
                orders_submitted=8, risk_blocks=2, errors=[],
            )
        await db.commit()
        a = (await db.execute(select(Alert).where(Alert.alert_type == "daily_summary"))).scalar_one()
        assert a.severity == "info"

    async def test_alert_daily_summary_with_errors(self, db):
        with _no_channels_ctx():
            await alert_daily_summary(
                db, date_str="2026-04-24", total_trades=5,
                orders_submitted=3, risk_blocks=2, errors=["order timeout"],
            )
        await db.commit()
        a = (await db.execute(select(Alert).where(Alert.alert_type == "daily_summary"))).scalar_one()
        assert a.severity == "warning"
    ...
    async def test_alert_feed_health_issue_stale(self, db):
        with _no_channels_ctx():
            await alert_feed_health_issue(
                db, status="stale", provider="polygon",
                detail="data >5m old", affected_symbols=["AAPL", "TSLA"],
            )
        await db.commit()
        a = (await db.execute(select(Alert).where(Alert.alert_type == "feed_health_issue"))).scalar_one()
        assert a.severity == "critical"

    async def test_alert_feed_health_issue_degraded(self, db):
        with _no_channels_ctx():
            await alert_feed_health_issue(
                db, status="degraded", provider="polygon",
                detail="slow", affected_symbols=[],
            )
        await db.commit()
        a = (await db.execute(select(Alert).where(Alert.alert_type == "feed_health_issue"))).scalar_one()
        assert a.severity == "warning"
    ...
    async def test_alert_feed_health_recovered(self, db):
        ...
        await db.commit()
        a = (await db.execute(select(Alert).where(Alert.alert_type == "feed_health_recovered"))).scalar_one()
        assert a.severity == "info"

    async def test_alert_regime_shift_risk_off(self, db):
        with _no_channels_ctx():
            await alert_regime_shift(
                db, previous_regime="normal", current_regime="risk_off",
                detail="VIX spike", suppressed_strategies=["ORB"],
            )
        await db.commit()
    ...
    async def test_alert_regime_shift_unsafe(self, db):
        with _no_channels_ctx():
            await alert_regime_shift(
                db, previous_regime="risk_off", current_regime="unsafe",
                detail="circuit break", suppressed_strategies=["ORB", "VWAP"],
            )
        await db.commit()
        a = (await db.execute(select(Alert).where(Alert.alert_type == "regime_shift"))).scalar_one()
        assert a.severity == "critical"
    ...
    async def test_alert_regime_shift_info(self, db):
        with _no_channels_ctx():
            await alert_regime_shift(
                db, previous_regime="risk_off", current_regime="normal",
                detail="VIX calmed", suppressed_strategies=[],
            )
        await db.commit()
        a = (await db.execute(select(Alert).where(Alert.alert_type == "regime_shift"))).scalar_one()
        assert a.severity == "info"
