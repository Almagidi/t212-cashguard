...
export const QK = {
  ...
  portfolioStrategyMonitoring: (id: string) => ['strategies', id, 'portfolio-monitoring'],
  portfolioAttribution: ['strategies', 'portfolio-attribution'],
  portfolioStrategyAttribution: (id: string) => ['strategies', id, 'portfolio-attribution'],
...
export const useMe = () => useQuery({ queryKey: QK.me, queryFn: api.getMe.bind(api), retry: false, staleTime: Infinity })

// ── Broker ──────────────────────────────────────────────────────────────────
export const useBrokerStatus = () =>
  useQuery({ queryKey: QK.brokerStatus, queryFn: api.getBrokerStatus.bind(api), refetchInterval: 30_000 })

export const useTestBroker = () => {
  const qc = useQueryClient()
  return useMutation<Awaited<ReturnType<typeof api.testBroker>>, any, void>({
    mutationFn: api.testBroker.bind(api),
    onSuccess: (data) => {
    ...
    onError: (e: any) => toast.error(extractErrorMessage(e, 'Connection test failed')),
  })
}
...
export const useDisconnectBroker = () => {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: api.disconnectBroker.bind(api),
    onSuccess: () => { qc.invalidateQueries({ queryKey: QK.brokerStatus }); toast.success('Broker disconnected') },
  })
}

// ── Account ──────────────────────────────────────────────────────────────────
export const useAccount = () =>
  useQuery({ queryKey: QK.account, queryFn: api.getAccountSummary.bind(api), refetchInterval: 15_000 })

export const useCashGuard = () =>
  useQuery({ queryKey: QK.cashGuard, queryFn: api.getCashGuardStatus.bind(api), refetchInterval: 15_000 })

// ── Instruments ─────────────────────────────────────────────────────────────
// Instrument list only changes on an explicit broker sync — cache for 5 min
export const useInstruments = (params?: Parameters<typeof api.getInstruments>[0]) =>
  useQuery({ queryKey: QK.instruments(params), queryFn: () => api.getInstruments(params), staleTime: 5 * 60_000 })

export const useSyncInstruments = () => {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: api.syncInstruments.bind(api),
    onSuccess: (data: any) => { qc.invalidateQueries({ queryKey: ['instruments'] }); toast.success(`Synced ${data.synced} instruments`) },
...
export const useStrategies = () =>
  useQuery({ queryKey: QK.strategies, queryFn: api.getStrategies.bind(api), staleTime: 30_000 })

// Presets are essentially static config — cache for 10 min
export const useStrategyPresets = () =>
  useQuery({ queryKey: QK.strategyPresets, queryFn: api.getStrategyPresets.bind(api), staleTime: 10 * 60_000 })

export const useStrategy = (id: string) =>
  useQuery({ queryKey: QK.strategy(id), queryFn: () => api.getStrategy(id), enabled: !!id, staleTime: 30_000 })
...
export const usePortfolioMonitoring = () =>
  useQuery({ queryKey: QK.portfolioMonitoring, queryFn: api.getPortfolioMonitoring.bind(api), refetchInterval: 30_000 })
...
export const usePortfolioAttribution = () =>
  useQuery({ queryKey: QK.portfolioAttribution, queryFn: api.getPortfolioAttribution.bind(api), refetchInterval: 30_000 })
...
export const useCreateStrategy = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: QK.strategies }); toast.success('Strategy created') },
...
export const useCreateStrategyPreset = () => {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ key, payload }: { key: StrategyPresetKey; payload?: CreateStrategyPresetPayload }) =>
    ...
    onError: (e: any) => toast.error(extractErrorMessage(e, 'Failed to create preset strategy')),
  })
}
...
export const useUpdateStrategyPromotion = () => {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ id, action, notes }: { id: string; action: StrategyPromotionAction; notes?: string }) =>
    ...
    onSuccess: (data, variables) => {
      ...
      qc.invalidateQueries({ queryKey: QK.strategies })
      const label = variables.action === 'promote_to_demo'
    ...
    onError: (e: any) => toast.error(extractErrorMessage(e, 'Strategy promotion update failed')),
  })
}

// ── Signals ─────────────────────────────────────────────────────────────────
export const useSignals = (params?: Parameters<typeof api.getSignals>[0]) =>
  useQuery({ queryKey: QK.signals(params), queryFn: () => api.getSignals(params), refetchInterval: 10_000 })

// ── Orders ──────────────────────────────────────────────────────────────────
export const useOrders = (params?: Parameters<typeof api.getOrders>[0]) =>
  useQuery({ queryKey: QK.orders(params), queryFn: () => api.getOrders(params), refetchInterval: 10_000 })
...
export const usePlaceOrder = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['orders'] }); qc.invalidateQueries({ queryKey: ['positions'] }); toast.success('Order placed') },
...
export const useCancelAllPending = () => {
  ...
  return useMutation({
    ...
    onSuccess: (d: any) => { qc.invalidateQueries({ queryKey: ['orders'] }); toast.success(`Cancelled ${d.cancelled} orders`) },
  })
}

// ── Positions ────────────────────────────────────────────────────────────────
export const usePositions = () =>
  useQuery({ queryKey: QK.positions, queryFn: api.getPositions.bind(api), refetchInterval: 15_000 })

// ── Risk ─────────────────────────────────────────────────────────────────────
// Risk profile only changes when explicitly saved — cache for 2 min
export const useRiskProfile = () =>
  useQuery({ queryKey: QK.riskProfile, queryFn: api.getRiskProfile.bind(api), staleTime: 2 * 60_000 })

export const useUpdateRiskProfile = () => {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (p: any) => api.updateRiskProfile(p),
    onSuccess: () => { qc.invalidateQueries({ queryKey: QK.riskProfile }); toast.success('Risk profile updated') },
    onError: () => toast.error('Update failed'),
  })
}

export const useRiskEvents = (params?: { limit?: number; event_type?: string }) =>
  useQuery({ queryKey: QK.riskEvents(params), queryFn: () => api.getRiskEvents(params), refetchInterval: 30_000 })
...
export const useKillSwitch = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_d, v) => {
      ...
      toast.success(v.active ? '⛔ Kill switch activated' : '✅ Kill switch deactivated')
    },
  })
}
...
export const useAlerts = (params?: { is_read?: boolean; limit?: number }) =>
  useQuery({ queryKey: QK.alerts(params), queryFn: () => api.getAlerts(params), refetchInterval: 30_000 })

// ── Settings ─────────────────────────────────────────────────────────────────
export const useSettings = () =>
  useQuery({ queryKey: QK.settings, queryFn: api.getSettings.bind(api), refetchInterval: 60_000 })

export const useUpdateSettings = () => {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: (p: any) => api.updateSettings(p),
    onSuccess: (data) => { qc.setQueryData(QK.settings, data); toast.success('Settings saved') },
  })
}

export const useLiveReadiness = () =>
  useQuery({ queryKey: QK.liveReadiness, queryFn: api.getLiveReadiness.bind(api), refetchInterval: 60_000 })

export const useUpdateLiveReadiness = () => {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ action, notes }: { action: Parameters<typeof api.updateLiveReadiness>[0]; notes?: string }) =>
      api.updateLiveReadiness(action, notes),
    onSuccess: (data, variables) => {
      qc.setQueryData(QK.liveReadiness, data)
      qc.invalidateQueries({ queryKey: QK.settings })
      const label = variables.action === 'unlock_live'
    ...
    onError: (e: any) => toast.error(extractErrorMessage(e, 'Live readiness update failed')),
  })
}

export const useTelegramStatus = () =>
  useQuery({ queryKey: QK.telegramStatus, queryFn: api.getTelegramStatus.bind(api), refetchInterval: 60_000 })
...
export const useTelegramTestAlert = () => {
  return useMutation({
    ...
    onError: (e: any) => toast.error(extractErrorMessage(e, 'Telegram test failed')),
  })
}
...
export const useEmergencyKillSwitch = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['settings'] }); toast.error('⛔ KILL SWITCH ACTIVATED') },
  })
}
export const useEmergencyAutoTradingOff = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['settings'] }); toast.success('Auto-trading disabled') },
  })
}
...
export const useEmergencyCancelAll = () => {
  ...
  return useMutation({
    ...
    onSuccess: (d: any) => { qc.invalidateQueries({ queryKey: ['orders'] }); toast.success(d.message) },
  })
}
export const useEmergencyFlattenAll = () => {
  ...
  return useMutation({
    ...
    onSuccess: (d: any) => { qc.invalidateQueries({ queryKey: ['positions', 'orders'] }); toast.success(d.message) },
  })
}

// ── Reports ──────────────────────────────────────────────────────────────────
export const usePerformanceReport = (days = 30) =>
  useQuery({ queryKey: [...QK.performance, days], queryFn: () => api.getPerformanceReport(days) })
...
export const useTradesList = (params?: { page?: number; page_size?: number; ticker?: string; has_notes?: boolean }) =>
  useQuery({ queryKey: QK.tradesList(params), queryFn: () => api.listTrades(params) })

export const useTrade = (id: string) =>
  useQuery({ queryKey: QK.trade(id), queryFn: () => api.getTrade(id), enabled: !!id })

export const useUpdateTradeJournal = () => {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: { notes?: string; tags?: string[]; emotion?: string; rating?: number } }) =>
...
export const useAuditLogs = (params?: Parameters<typeof api.getAuditLogs>[0]) =>
  useQuery({ queryKey: QK.auditLogs(params), queryFn: () => api.getAuditLogs(params), refetchInterval: 30_000 })

// ── Health ───────────────────────────────────────────────────────────────────
export const useHealth = () =>
  useQuery({ queryKey: QK.health, queryFn: api.getHealth.bind(api), refetchInterval: 30_000 })

export const useDepsHealth = () =>
  useQuery({ queryKey: QK.depsHealth, queryFn: api.getDepsHealth.bind(api), refetchInterval: 30_000 })

export const useMarketDataHealth = () =>
  useQuery({ queryKey: QK.marketDataHealth, queryFn: api.getMarketDataHealth.bind(api), refetchInterval: 30_000 })

export const useMarketRegime = () =>
  useQuery({ queryKey: QK.regime, queryFn: api.getMarketRegime.bind(api), refetchInterval: 30_000 })
