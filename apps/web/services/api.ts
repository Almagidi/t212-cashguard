...
import type {
  AccountSummary, Alert, AppSettings, AuditLogList,
  ...
  Order, OrderDetail, PerformanceReport, Position,
  PortfolioStrategyAttribution, PortfolioStrategyAttributionSummary, PortfolioStrategyMonitoring,
  PortfolioBacktestJob, PortfolioBacktestRunRequest, PortfolioBacktestStrategyInfo,
  RiskEvent, RiskProfile, Signal, Strategy, StrategyIntelligence,
  StrategyPresetInfo, StrategyPromotionAction, StrategyPromotionStatus,
  StrategyDryRunResult, TelegramStatus, TelegramTestResult, User,
...
class ApiClient {
  ...
  constructor() {
    ...
    this.client.interceptors.response.use(
      ...
      async (error: AxiosError) => {
        ...
        if (is401 && hasToken && !isAuthEndpoint && !_verifyingLogout && typeof window !== 'undefined') {
  ...
  // ── Broker ──────────────────────────────────────────────────────────────────
  async connectBroker(payload: { api_key: string; api_secret: string; environment: 'demo' | 'live' }): Promise<BrokerStatus> {
    return (await this.client.post<BrokerStatus>('/broker/trading212/connect', payload)).data
  }
  async testBroker(): Promise<BrokerTestResult> {
    return (await this.client.post<BrokerTestResult>('/broker/trading212/test')).data
  }
  async disconnectBroker(): Promise<void> {
    await this.client.delete('/broker/trading212/disconnect')
  }
  async getBrokerStatus(): Promise<BrokerStatus | null> {
    return (await this.client.get<BrokerStatus | null>('/broker/trading212/status')).data
  }
  ...
  // ── Instruments ─────────────────────────────────────────────────────────────
  async getInstruments(params?: { search?: string; type?: string; page?: number; page_size?: number }): Promise<InstrumentList> {
    return (await this.client.get<InstrumentList>('/instruments', { params })).data
  }
  ...
  async getStrategyPresets(): Promise<StrategyPresetInfo[]> {
    return (await this.client.get<StrategyPresetInfo[]>('/strategies/presets')).data
  }
  ...
  async createStrategyFromPreset(key: string, payload: CreateStrategyPresetPayload = {}): Promise<Strategy> {
    return (await this.client.post<Strategy>(`/strategies/presets/${key}`, payload)).data
  }
  async getStrategy(id: string): Promise<Strategy> {
    return (await this.client.get<Strategy>(`/strategies/${id}`)).data
  }
  async getStrategyPromotionStatus(id: string): Promise<StrategyPromotionStatus> {
    return (await this.client.get<StrategyPromotionStatus>(`/strategies/${id}/promotion-status`)).data
  }
  async updateStrategyPromotion(id: string, action: StrategyPromotionAction, notes?: string): Promise<StrategyPromotionStatus> {
    return (await this.client.post<StrategyPromotionStatus>(`/strategies/${id}/promotion`, { action, notes })).data
  }
  async getPortfolioMonitoring(): Promise<PortfolioStrategyMonitoring[]> {
    return (await this.client.get<PortfolioStrategyMonitoring[]>('/strategies/portfolio-monitoring')).data
  }
  async getPortfolioStrategyMonitoring(id: string): Promise<PortfolioStrategyMonitoring> {
    return (await this.client.get<PortfolioStrategyMonitoring>(`/strategies/${id}/portfolio-monitoring`)).data
  }
  async getPortfolioAttribution(): Promise<PortfolioStrategyAttributionSummary[]> {
    return (await this.client.get<PortfolioStrategyAttributionSummary[]>('/strategies/portfolio-attribution')).data
  }
  async getPortfolioStrategyAttribution(id: string): Promise<PortfolioStrategyAttribution> {
    return (await this.client.get<PortfolioStrategyAttribution>(`/strategies/${id}/portfolio-attribution`)).data
  }
  async getStrategyIntelligence(id: string): Promise<StrategyIntelligence> {
    return (await this.client.get<StrategyIntelligence>(`/strategies/${id}/intelligence`)).data
  }
  async updateStrategy(id: string, payload: Partial<CreateStrategyPayload>): Promise<Strategy> {
    return (await this.client.patch<Strategy>(`/strategies/${id}`, payload)).data
  }
  ...
  async dryRunStrategy(id: string): Promise<StrategyDryRunResult> {
    return (await this.client.post<StrategyDryRunResult>(`/strategies/${id}/run-dry`)).data
  }
  ...
  async listBacktestStrategies(): Promise<BacktestStrategyInfo[]> {
    return (await this.client.get<BacktestStrategyInfo[]>('/backtest/strategies')).data
  }
  async listPortfolioBacktestStrategies(): Promise<PortfolioBacktestStrategyInfo[]> {
    return (await this.client.get<PortfolioBacktestStrategyInfo[]>('/backtest/portfolio/strategies')).data
  }
  async runBacktest(payload: BacktestRunRequest): Promise<BacktestJobResponse> {
    return (await this.client.post<BacktestJobResponse>('/backtest/run', payload)).data
  }
  async getBacktestResult(jobId: string): Promise<BacktestJob> {
    return (await this.client.get<BacktestJob>(`/backtest/result/${jobId}`)).data
  }
  async runPortfolioBacktest(payload: PortfolioBacktestRunRequest): Promise<BacktestJobResponse> {
    return (await this.client.post<BacktestJobResponse>('/backtest/portfolio/run', payload)).data
  }
  async getPortfolioBacktestResult(jobId: string): Promise<PortfolioBacktestJob> {
    return (await this.client.get<PortfolioBacktestJob>(`/backtest/portfolio/result/${jobId}`)).data
  }

  // ── Signals ─────────────────────────────────────────────────────────────────
  async getSignals(params?: { status?: string; ticker?: string; limit?: number }): Promise<Signal[]> {
    return (await this.client.get<Signal[]>('/signals', { params })).data
  }

  // ── Orders ──────────────────────────────────────────────────────────────────
  async getOrders(params?: { status?: string; ticker?: string; limit?: number }): Promise<Order[]> {
  ...
  async updateRiskProfile(payload: Partial<RiskProfile>): Promise<RiskProfile> {
    return (await this.client.patch<RiskProfile>('/risk/profile', payload)).data
  }
  async getRiskEvents(params?: { limit?: number; event_type?: string }): Promise<RiskEvent[]> {
    return (await this.client.get<RiskEvent[]>('/risk/events', { params })).data
  }
  ...
  // ── Alerts ──────────────────────────────────────────────────────────────────
  async getAlerts(params?: { is_read?: boolean; limit?: number }): Promise<Alert[]> {
  ...
  async getLiveReadiness(): Promise<LiveReadinessStatus> {
    return (await this.client.get<LiveReadinessStatus>('/settings/live-readiness')).data
  }
  async updateLiveReadiness(action: LiveReadinessAction, notes?: string): Promise<LiveReadinessStatus> {
    return (await this.client.post<LiveReadinessStatus>('/settings/live-readiness', { action, notes })).data
  }
  ...
  async sendTelegramTestAlert(): Promise<TelegramTestResult> {
    return (await this.client.post<TelegramTestResult>('/telegram/test-alert')).data
  }

  // ── Emergency ───────────────────────────────────────────────────────────────
  async emergencyKillSwitch(): Promise<EmergencyActionResult> {
    return (await this.client.post<EmergencyActionResult>('/emergency/kill-switch')).data
  }
  async emergencyAutoTradingOff(): Promise<EmergencyActionResult> {
    return (await this.client.post<EmergencyActionResult>('/emergency/auto-trading/off')).data
  }
  async emergencyAutoTradingOn(): Promise<EmergencyActionResult> {
    return (await this.client.post<EmergencyActionResult>('/emergency/auto-trading/on')).data
  }
  async emergencyCancelAll(): Promise<EmergencyActionResult> {
    return (await this.client.post<EmergencyActionResult>('/emergency/cancel-all')).data
  }
  async emergencyFlattenAll(): Promise<EmergencyActionResult> {
    return (await this.client.post<EmergencyActionResult>('/emergency/flatten-all')).data
  }

  // ── Reports ─────────────────────────────────────────────────────────────────
  async getPerformanceReport(days = 30): Promise<PerformanceReport> {
    return (await this.client.get<PerformanceReport>('/reports/performance', { params: { days } })).data
  }
  ...
  // Trade Journal
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  async listTrades(params?: { page?: number; page_size?: number; ticker?: string; has_notes?: boolean }): Promise<any> {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  async updateTradeJournal(id: string, payload: { notes?: string; tags?: string[]; emotion?: string; rating?: number }): Promise<any> {
    return (await this.client.patch(`/trades/${id}/journal`, payload)).data
  }

  // ── Audit ────────────────────────────────────────────────────────────────────
  async getAuditLogs(params?: { action?: string; entity_type?: string; page?: number; page_size?: number }): Promise<AuditLogList> {
  ...
  async getWatchlistIntelligence(limit = 8): Promise<WatchlistIntelligence> {
    return (await this.client.get<WatchlistIntelligence>('/intelligence/watchlist', { params: { limit } })).data
  }
  ...
  async getMarketDataHealth(): Promise<MarketDataHealth> {
    return (await this.client.get<MarketDataHealth>('/health/market-data')).data
  }
}
