...
export interface User { id: string; email: string; is_active: boolean; is_admin: boolean; created_at: string }

// ── Broker ────────────────────────────────────────────────────────────────────
export type Environment = 'demo' | 'live' | 'mock'
export type BrokerCredentialState = 'mock' | 'configured' | 'reconnect_required' | 'not_connected'
export type BrokerDiagnosticLikelihood = 'likely' | 'possible'
export type BrokerDiagnosticKey = 'wrong_environment' | 'invalid_credentials' | 'ip_restriction'
...
export interface BrokerStatus {
  id: string; broker: string; environment: Environment; is_active: boolean
  credential_state: BrokerCredentialState; recovery_hint: string | null
  last_test_at: string | null; last_test_ok: boolean | null
  last_sync_at: string | null; account_id: string | null; account_currency: string | null
  created_at: string
}
export interface BrokerTestResult {
  is_ok: boolean; account_id: string | null; currency: string | null; error: string | null
  diagnostics: BrokerDiagnostics | null
}

// ── Account ───────────────────────────────────────────────────────────────────
export interface AccountSummary {
  total_value: number; cash: number; free_funds: number
  invested: number; result: number; currency: string
  synced_at: string | null; mode: string
}
export interface CashGuardStatus {
  available_to_trade: number; reserved: number; total_cash: number
  cash_only_mode: boolean; currency: string
}

// ── Instruments ───────────────────────────────────────────────────────────────
export interface Instrument {
  id: string; ticker: string; name: string; type: string; currency_code: string
  isin: string | null; extended_hours: boolean; working_schedule_id: number | null
  trading_enabled: boolean; synced_at: string | null
}
export interface InstrumentList { items: Instrument[]; total: number }

// ── Risk ──────────────────────────────────────────────────────────────────────
export interface RiskProfile {
  id: string; name: string; max_risk_per_trade_pct: string; max_daily_loss_pct: string
  max_open_positions: number; max_position_size_pct: string; max_trades_per_day: number
  stop_after_consecutive_losses: number; symbol_cooldown_seconds: number
  force_flat_eod: boolean; is_default: boolean; created_at: string; updated_at: string
}
...
export interface Strategy {
  id: string; name: string; type: StrategyType; description: string | null
  is_enabled: boolean; is_live: boolean; risk_profile_id: string | null
  risk_profile?: RiskProfile | null
  params: Record<string, unknown>; allowed_tickers: string[]
  session_start: string; session_end: string; extended_hours: boolean
  eod_flatten: boolean; last_signal_at: string | null; created_at: string; updated_at: string
}
...
export interface CreateStrategyPayload {
  name: string; type: StrategyType; description?: string
  params?: Record<string, unknown>; allowed_tickers?: string[]
  session_start?: string; session_end?: string
  extended_hours?: boolean; eod_flatten?: boolean
  risk_profile_id?: string; is_live?: boolean
}
...
export interface PortfolioStrategyAttribution extends PortfolioStrategyAttributionSummary {
...
export interface Signal {
  id: string; strategy_id: string; strategy_name: string | null; strategy_type_name: string | null; ticker: string; side: 'buy' | 'sell'
  signal_type: string; status: string; entry_price: string | null
  stop_price: string | null; take_profit_price: string | null
  suggested_quantity: string | null; confidence: string | null
  reason: string | null; risk_rejected: boolean; risk_rejection_reason: string | null
  params_snapshot: Record<string, unknown> | null
  generated_at: string; expires_at: string | null; executed_at: string | null
}

// ── Orders ────────────────────────────────────────────────────────────────────
export type OrderSide = 'buy' | 'sell'
export type OrderType = 'market' | 'limit' | 'stop' | 'stop_limit'
export type OrderStatus = 'pending_intent' | 'submitted' | 'accepted' | 'filled' | 'cancelled' | 'rejected' | 'error'
export interface Order {
  id: string; signal_id: string | null; strategy_id: string | null; strategy_name: string | null; strategy_type_name: string | null; client_order_key: string
  ticker: string; side: OrderSide; order_type: OrderType; quantity: string
...
export interface CreateOrderPayload {
  ticker: string; side: OrderSide; order_type: OrderType; quantity: string
  limit_price?: string; stop_price?: string; time_validity?: 'DAY' | 'GTC'
  signal_id?: string
}
...
export interface Alert {
  id: string; alert_type: string; channel: string; title: string
  message: string; severity: 'info' | 'warning' | 'error' | 'critical'
  is_read: boolean; created_at: string
}

// ── Settings ──────────────────────────────────────────────────────────────────
export interface AppSettings {
  id: number; theme: string; timezone: string; market_data_provider: string
  auto_trading_enabled: boolean; kill_switch_active: boolean
  live_trading_unlocked: boolean; daily_stats_reset_time: string; updated_at: string
}
...
export interface AuditLog {
  ...
  entity_id: string | null; actor: string; ip_address: string | null
  payload: Record<string, unknown> | null; occurred_at: string
}
export interface AuditLogList { items: AuditLog[]; total: number; page: number; page_size: number }

// ── Risk Events ───────────────────────────────────────────────────────────────
export interface RiskEvent { id: string; event_type: string; ticker: string | null; message: string | null; occurred_at: string }
...
export interface PerformanceReport {
  ...
  profit_factor: number; max_drawdown: number; sharpe_ratio: number | null
  daily_pnl: Array<{ date: string; pnl: number }>
}

// ── Health ────────────────────────────────────────────────────────────────────
export interface HealthStatus { status: string; timestamp: string; version: string; mode: string }
export interface DepsHealth { database: string; redis: string; broker: string; market_data: string }
...
// ── Emergency ─────────────────────────────────────────────────────────────────
export interface EmergencyActionResult { success: boolean; action: string; message: string; timestamp: string }

// ── Pagination ────────────────────────────────────────────────────────────────
export interface PaginatedList<T> { items: T[]; total: number; page: number; page_size: number }
