...
PositionMonitor now uses the Trading 212 provider for final adapter construction. `PositionMonitor._get_broker` still preserves the existing mock-mode shortcut, active encrypted connection lookup, credential decryption, reconnect-required marking with actor `position_monitor`, and `require_broker_environment(conn.environment, action="position monitor broker access")` before provider construction. The provider request uses purpose `worker_position_monitor`; it is user-scoped and write-capable because position/account reads, automated exits, and EOD flatten order production share the same broker helper. Tests prove no provider call in mock/no-connection/decryption-failure/policy-rejection cases, prove provider validation failure returns no broker without logging secrets, preserve the current mock-mode dry-run flag for monitor-produced orders, and prove automated exits route through `ExecutionEngine` while EOD flatten delegates to the scoped `EodFlattenService`. This is not a live-readiness claim and does not add frontend controls, route/schema changes, Kraken/Alpaca support, real broker calls in tests, cancellation changes, or order-placement changes.

Position-monitor daily-loss safety now uses non-dry-run closed `Trade.realized_pnl` for realised P&L instead of order cash-flow fields and applies `POSITION_MONITOR_UNREALIZED_PNL_FAILURE_POLICY` when unrealized P&L cannot be calculated.

## End-of-Day Flatten

The scheduled EOD task preserves the kill-switch check and Redis overlap lock, but durable database state is the correctness boundary. It passes every enabled, live, EOD-enabled strategy and an aware UTC instant to `PositionMonitor`, which avoids broker construction until at least one strategy is inside its bounded due window.

The current explicit runtime-to-exchange mapping is `t212 -> XNYS`. Strategy `session_end` is interpreted as an exchange-local `HH:MM`. The effective cutoff is the earlier of the configured cutoff and the authoritative exchange close, so XNYS holidays do not run and early closes use the calendar close. A task is due only from the cutoff through the exclusive end of a ten-minute window. Unsupported venues, including the deferred Kraken runtime, fail closed.

Every attempted liquidation is scoped to filled, signal-linked orders for one strategy and ticker, in the current execution environment and explicit broker-account scope, less filled prior EOD-operation orders for that same scope. Orders now persist the non-secret, hashed account scope assigned by the validated provider boundary from the authenticated user and broker-reported account ID. Reconnecting the same user/environment to a different Trading 212 account produces a different scope, so old fills cannot authorize a sell in the replacement account. Legacy broker-backed fills or adapters without an authoritative account ID fail closed for EOD. A single broker snapshot caps the aggregate sellable quantity when strategies share a ticker. Manual holdings are not included. Missing or duplicate broker rows, insufficient broker quantity or `maxSell`, malformed fill quantities, and pre-existing active sells submit no order and create a durable manual-reconciliation operation, risk event, and in-app operator alert.

Before broker transmission, the service commits both `eod_flatten_operations` and its order intent. The unique operation identity is `(operation_kind, strategy_id, venue, exchange_session_date, ticker)`, and the order client key is deterministically derived from the same identity. Task redelivery, process restart, concurrent workers, terminal local orders, and delayed broker settlement therefore cannot create another sell for that identity. Unresolved operations block later sessions for the same strategy/venue/ticker until explicit reconciliation. Linked orders that later fill are promoted to completed operations; linked terminal failures become manual-reconciliation incidents. The global kill switch is re-read from the database immediately before submission. A positive attributable quantity that reappears after a completed same-session flatten is marked `reentry_blocked`, emits risk/operator evidence, and is not liquidated again automatically.

EOD broker submission remains disabled in live mode. The mock service path records manual-reconciliation evidence instead of claiming that the separate `PaperExecutionEngine` ledger was flattened. The scheduled paper worker fails closed before service construction when no authoritative paper broker exists. Kraken remains outside this EOD implementation.
