...
System-control provider-equivalence coverage is tests/docs-only. `SystemControlService._get_broker` remains the remaining major direct Trading 212 construction path and is not provider-backed yet. It still directly constructs `Trading212Adapter` after the existing mock-mode shortcut, app-mode policy gate, active encrypted connection lookup, connection-environment gate, and credential decryption. This is intentionally deferred because one helper serves both read/status operator visibility and emergency operator actions. Read/status methods are proven read-only against fakes, while emergency cancel-all and flatten-all are proven write-capable only through fake broker and fake execution-engine records. Emergency operations are not read-only. No runtime provider migration, live-readiness claim, real broker call, network call, cancellation, or order placement is added.
...
- Keep `SystemControlService._get_broker` direct/provider-unwired until a later runtime migration splits read-only operator status from emergency operator actions, or gives those surfaces separate provider purposes and tests. Emergency cancel/flatten operations must stay classified as write-capable. `cancel_timed_out_orders`, PositionMonitor, StrategyRunner, and PortfolioExecutionService are no longer remaining direct construction paths, but future edits must preserve their `worker_cancel_timed_out_orders`, `worker_position_monitor`, `worker_strategy_runner`, and `worker_portfolio_execution` purposes, user-scoped active credential source, fake broker boundaries, risk gates, and `ExecutionEngine` order routing behavior.
