...
- Add a tests-only/equivalence PR for exactly one remaining direct provider candidate before any runtime construction migration. For system control, split read-only status from emergency-write paths or give them separate provider purposes and tests before changing runtime construction. PositionMonitor, StrategyRunner, and PortfolioExecutionService are no longer remaining direct construction paths, but future edits must preserve their `worker_position_monitor`, `worker_strategy_runner`, and `worker_portfolio_execution` purposes, user-scoped active credential source, fake broker boundaries, risk gates, and `ExecutionEngine` order submission behavior.
