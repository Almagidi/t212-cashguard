...
EOD broker submission remains disabled in live mode. Mock/paper EOD also fails closed with manual evidence because its authoritative position ledger is maintained by `PaperExecutionEngine`; the generic dry-run execution path is not allowed to claim that ledger was flattened. Kraken remains outside this EOD implementation.
