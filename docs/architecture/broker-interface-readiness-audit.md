...
- scheduler startup, the terminal one-shot demo reconciliation worker, `sync_account_snapshot`, `track_cfd_funding`, `reconcile_pending_orders`, and `PositionMonitor._get_broker` delegate final adapter construction to the provider after their existing gates.
...
- `apps/api/app/services/system_control.py`, `apps/api/app/services/strategy_runner.py`, `apps/api/app/services/portfolio_execution_service.py`, and selected functions in `apps/api/app/workers/tasks.py`: construct Trading 212 adapters for operational broker reads/writes.
...
This audit covers the remaining direct `Trading212Adapter` construction/import paths after the provider migrations. The latest update migrates `PositionMonitor._get_broker` final adapter construction to the provider while keeping the monitor classified as write-capable. It does not change routes or schemas, change credential storage/decryption, place broker orders, cancel broker orders, expand live trading, add Alpaca/Kraken, add frontend order controls, or weaken safety gates.
...
`apps/api/tests/unit/test_strategy_runner_provider_equivalence.py` adds focused equivalence coverage for the current `StrategyRunner` boundary without migrating runtime construction. It locks `_get_broker` as direct/provider-unwired, proves mock mode, no active connection, credential-decryption failure, policy rejection, active encrypted credential use, and construction order against fakes, and verifies `require_broker_environment(...)` still runs after credential decryption and before direct `Trading212Adapter` construction. It also proves `run_all_enabled(...)` kill-switch and auto-trading skips happen before broker lookup, proves strategy runner remains mixed/write-capable because account/position reads and strategy entry/exit order production share the same broker helper, preserves dry-run entry/exit behavior, and proves live entry/exit order-producing paths route through a fake `ExecutionEngine` only. No provider helper is called, and no real broker, network, cancellation, or order submission occurs in these tests.
...
| `apps/api/app/services/strategy_runner.py` | `StrategyRunner._get_broker` | Local import and direct construction. | Reads account/positions and can submit strategy orders through `ExecutionEngine`. | Mock returns mock adapter; active connection for `settings.APP_MODE`; decryption failure marks reconnect required; `require_broker_environment(conn.environment, action="strategy runner broker access")`; higher-level auto-trading, kill switch, strategy state, venue, promotion, allocation, and risk checks before submission. | Active encrypted `BrokerConnection` for current `settings.APP_MODE`. | Mixed/write-capable. | Later. Not a safe first target because entry and exit submission, dry-run behavior, strategy gates, venue gates, promotion gates, allocation gates, and risk gates must be proven equivalent together. | A generic provider path could make strategy writes easier to reach before the strategy/live gates are proven equivalent. | Current provider-equivalence tests lock account/position reads, no-broker/decryption/environment skips, dry-run entry/exit behavior, and fake-engine entry/exit submission. Before runtime migration, add or retain focused venue, promotion, allocation, and risk blocker coverage. |
...
- provider receives `broker_id="trading212"`, the active connection environment, caller-specific purpose (`worker_account_sync`, `worker_cfd_funding`, `worker_reconcile`, or `worker_position_monitor`), and the active connection user id;
...
As of 2026-05-31, the provider-backed production construction paths are `get_broker()`, `/v1/broker/trading212` credential-test construction, scheduler startup, the terminal one-shot demo reconciliation worker, `sync_account_snapshot`, `track_cfd_funding`, `reconcile_pending_orders`, and `PositionMonitor._get_broker`.
...
`strategy_runner` now has tests/docs-only provider-equivalence coverage for the existing direct boundary. It remains direct/provider-unwired because account/position reads and strategy entry/exit order production share `_get_broker`; no runtime migration has happened. The current coverage locks construction paths, no-broker/decryption/environment skips, early `run_all_enabled(...)` kill-switch and auto-trading gates, account/position reads, dry-run entry and exit behavior, and live entry/exit order-producing behavior through fake `ExecutionEngine` calls only. Before provider wiring, add or retain focused coverage for strategy enabled filtering, venue gates, promotion gates, allocation rejection, and risk blockers, then preserve active encrypted credential source, provider purpose, fake broker boundary, and the fact that order-producing behavior is routed through `ExecutionEngine` rather than direct broker write calls.
...
14. Before migrating any remaining direct write-capable or mixed path, add a tests-only/equivalence PR for exactly one chosen candidate. The PR should lock existing safety gates, credential source, direct/provider-unwired baseline, provider request purpose, fake broker boundary, and unchanged read/write behavior.
15. Only after the remaining direct construction paths are migrated where appropriate, design a second adapter spike using recorded/non-live fixtures. Do not add live trading or strategy-driven broker writes as part of that spike.

## Next Recommended PR

Keep remaining write-capable paths deferred until each has focused acceptance coverage. The next roadmap item should be StrategyRunner provider migration prep: retain or deepen tests for no-broker/decryption/environment skips, venue/promotion/allocation/risk blockers, dry-run/live behavior, fake broker boundaries, and `ExecutionEngine` entry/exit routing before changing runtime construction. `cancel_timed_out_orders` should remain deferred because it can cancel broker orders through `ExecutionEngine.cancel_order(...)`.
