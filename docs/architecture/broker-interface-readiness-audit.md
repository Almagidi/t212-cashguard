...
- `apps/api/app/services/system_control.py`, `apps/api/app/services/portfolio_execution_service.py`, and selected functions in `apps/api/app/workers/tasks.py`: construct Trading 212 adapters for operational broker reads/writes.
...
This audit covers the remaining direct `Trading212Adapter` construction/import paths after the provider migrations. The latest update migrates `StrategyRunner._get_broker` final adapter construction to the provider while keeping the runner classified as write-capable and adding an explicit live-unlocked runtime skip. It does not change routes or schemas, change credential storage/decryption, place broker orders, cancel broker orders, expand live trading, add Alpaca/Kraken, add frontend order controls, or weaken safety gates.
...
`apps/api/tests/unit/test_portfolio_execution_provider_equivalence.py` adds focused equivalence coverage for the current `PortfolioExecutionService` boundary without migrating runtime construction. It locks `_get_broker` as direct/provider-unwired, proves mock mode, no active connection, credential-decryption failure, policy rejection, active encrypted credential use, and construction order against fakes, and verifies `require_broker_environment(...)` still runs after credential decryption and before direct `Trading212Adapter` construction. It also proves `run_all_enabled(...)` kill-switch, auto-trading, and live-unlocked skips happen before broker lookup, and proves portfolio execution remains mixed/write-capable: account and position reads happen through the shared broker helper, while dry-run rebalance order intent and submission flow through a fake `ExecutionEngine` only. No real broker, network, cancellation, or order submission occurs in these tests.
...
`portfolio_execution_service` now has tests/docs-only provider-equivalence coverage for the existing direct boundary. It remains direct/provider-unwired because account/position reads and rebalance order submission share `_get_broker`; no runtime migration has happened. The current coverage locks construction paths, early `run_all_enabled(...)` safety gates, account/position reads, dry-run order routing, and the live promotion gate. Before provider wiring, add or retain focused coverage for allocation and risk blocker paths, then preserve active encrypted credential source, provider purpose, fake broker boundary, and the fact that order-producing behavior is routed through `ExecutionEngine` rather than direct broker write calls.
...
16. Only after the remaining direct construction paths are migrated where appropriate, design a second adapter spike using recorded/non-live fixtures. Do not add live trading or strategy-driven broker writes as part of that spike.

## Next Recommended PR

Keep remaining write-capable paths deferred until each has focused acceptance coverage. The next roadmap item should choose exactly one remaining direct candidate, likely PortfolioExecutionService or SystemControlService, and retain or deepen tests for no-broker/decryption/environment skips, domain-specific risk blockers, dry-run/live behavior, fake broker boundaries, and `ExecutionEngine` routing before changing runtime construction. `cancel_timed_out_orders` should remain deferred because it can cancel broker orders through `ExecutionEngine.cancel_order(...)`.
