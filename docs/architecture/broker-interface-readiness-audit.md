...
- scheduler startup, the terminal one-shot demo reconciliation worker, `sync_account_snapshot`, `track_cfd_funding`, `reconcile_pending_orders`, `PositionMonitor._get_broker`, and `StrategyRunner._get_broker` delegate final adapter construction to the provider after their existing gates.
- selected service helpers, selected worker tasks, and controlled smoke scripts still construct `Trading212Adapter` directly as inventoried below.
...
- `apps/api/app/services/system_control.py`: constructs Trading 212 adapters for shared status and emergency broker operations.
...
The next recommended provider step, if the migration continues, is another tests-only/equivalence PR for exactly one remaining direct candidate. `SystemControlService` remains direct/deferred. That PR should prove the selected path's existing safety gates, credential source, direct/provider-unwired baseline, provider request purpose, fake broker boundary, and unchanged read/write behavior before any runtime construction changes are made.
...
Keep remaining write-capable paths deferred until each has focused acceptance coverage. The next roadmap item should choose exactly one remaining direct candidate, likely SystemControlService, and retain or deepen tests for no-broker/decryption/environment skips, domain-specific blockers, fake broker boundaries, and `ExecutionEngine` routing before changing runtime construction. `cancel_timed_out_orders` is now provider-backed but must remain classified as write-capable because it can cancel broker orders through `ExecutionEngine.cancel_order(...)`.
