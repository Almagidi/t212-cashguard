...
- scheduler startup, the terminal one-shot demo reconciliation worker, and `sync_account_snapshot` delegate final adapter construction to the provider after their existing gates.
...
This audit covers the remaining direct `Trading212Adapter` construction/import paths after the provider migrations. The latest update migrates only `sync_account_snapshot`; it does not change routes or schemas, change credential storage/decryption, place broker orders, expand live trading, add Alpaca, add frontend order controls, or weaken safety gates.
...
`sync_account_snapshot` in `apps/api/app/workers/tasks.py` now uses `create_trading212_provider_adapter(...)` only for final Trading 212 adapter construction. It remains read-only at the broker boundary and calls only `get_account_summary()`.

Active connection lookup, credential decryption, reconnect-required handling, `require_broker_environment(conn.environment, action="worker account sync")`, provider-validation failure summarization, and `BrokerAccountSnapshot` persistence remain in worker code. The provider still does not query the database, decrypt credentials, read settings, choose credentials, persist snapshots, audit worker events, or place/cancel/modify orders.
...
Read-only/account-sync migration now locks these acceptance tests:
...
11. Only after the remaining direct construction paths are migrated where appropriate, design a second adapter spike using recorded/non-live fixtures. Do not add live trading or strategy-driven broker writes as part of that spike.

## Next Recommended PR

Keep write-capable paths deferred. The next candidate may be another read-only path such as `track_cfd_funding`, but it should be handled in a separate narrow PR with provider-equivalence tests for its mock/no-connection/decryption/environment gates and positions-to-records behavior.
