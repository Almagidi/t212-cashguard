...
`system_control` now has tests/docs-only provider-equivalence coverage for the existing direct boundary. It remains the remaining major direct Trading 212 construction path and is intentionally direct/provider-unwired because read/status paths and emergency cancel/flatten paths share `_get_broker`. Any future migration should split read-only operator status from emergency operator actions, or introduce separate provider purposes with separate acceptance tests, before changing runtime construction. Emergency cancel/flatten operations are not read-only.
...
The next recommended provider step, if the migration continues, is a runtime-migration design for `SystemControlService` that preserves this tests/docs-only baseline. `SystemControlService` remains direct/deferred. That future work should split read-only operator status from emergency write-capable operator actions, or give those surfaces separate provider purposes and acceptance tests, before any runtime construction changes are made.
