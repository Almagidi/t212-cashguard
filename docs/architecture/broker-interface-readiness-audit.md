...
- scheduler startup, the terminal one-shot demo reconciliation worker, `sync_account_snapshot`, `track_cfd_funding`, `reconcile_pending_orders`, `cancel_timed_out_orders`, `PositionMonitor._get_broker`, `StrategyRunner._get_broker`, and `PortfolioExecutionService._get_broker` delegate final adapter construction to the provider after their existing gates.
- `SystemControlService._get_broker` remains the major direct production Trading 212 construction path; controlled smoke scripts also still construct `Trading212Adapter` directly as inventoried below.
...
- `apps/api/app/services/system_control.py`: the remaining major direct production construction path. It constructs Trading 212 adapters for shared status and emergency broker operations and is intentionally not provider-backed yet.
...
This audit covers the remaining direct `Trading212Adapter` construction/import paths after the provider migrations. The latest update migrates `cancel_timed_out_orders` final adapter construction to the provider while keeping the worker classified as write-capable because cancellation still routes through `ExecutionEngine.cancel_order(...)`. It does not change routes or schemas, change credential storage/decryption, change timeout selection, place broker orders, alter cancellation semantics, expand live trading, add Alpaca/Kraken, add frontend order controls, or weaken safety gates.
...
`apps/api/tests/unit/test_write_capable_provider_boundary_audit.py` adds the semantic boundary lock for the remaining paths. It proves `cancel_timed_out_orders` is provider-backed with purpose `worker_cancel_timed_out_orders` and cancellation-capable, proves provider usage in `apps/api/app/workers/tasks.py` remains limited to `sync_account_snapshot`, `track_cfd_funding`, `reconcile_pending_orders`, and `cancel_timed_out_orders`, proves `PositionMonitor._get_broker` is provider-backed with purpose `worker_position_monitor`, proves `StrategyRunner._get_broker` is provider-backed with purpose `worker_strategy_runner`, proves `PortfolioExecutionService._get_broker` is provider-backed with purpose `worker_portfolio_execution`, and classifies `position_monitor`, `strategy_runner`, `portfolio_execution_service`, and `system_control` by their write-capable surfaces. The test is source/AST-level only and does not execute broker code.
...
| --- | --- |
| Runtime production app paths | `app/broker/provider.py`; `app/services/portfolio_execution_service.py`; `app/services/position_monitor.py`; `app/services/strategy_runner.py`; `app/services/system_control.py` |
...
Remaining direct write-capable execution paths should remain deferred. `system_control` emergency actions can submit or cancel orders after their own domain gates. Moving that broad helper too early would make it harder to prove the provider migration did not change order placement, cancellation, dry-run behavior, live unlock semantics, risk gates, or kill-switch coverage.
...
- provider receives `broker_id="trading212"`, the active connection environment, caller-specific purpose (`worker_account_sync`, `worker_cfd_funding`, `worker_reconcile`, `worker_cancel_timed_out_orders`, `worker_position_monitor`, or `worker_strategy_runner`), and the active connection user id;
...
As of 2026-05-31, the provider-backed production construction paths are `get_broker()`, `/v1/broker/trading212` credential-test construction, scheduler startup, the terminal one-shot demo reconciliation worker, `sync_account_snapshot`, `track_cfd_funding`, `reconcile_pending_orders`, `cancel_timed_out_orders`, `PositionMonitor._get_broker`, `StrategyRunner._get_broker`, and `PortfolioExecutionService._get_broker`.
...
- `system_control`: mixed read/status plus emergency cancel/flatten path; direct/provider-unwired. Future provider purposes exist: `operator_system_control_read` for read/status and `operator_system_control_emergency` for write-capable emergency actions.
...
`system_control` now has tests/docs-only provider-equivalence coverage for the existing direct boundary. It remains the remaining major direct Trading 212 construction path and is intentionally direct/provider-unwired because read/status paths and emergency cancel/flatten paths share `_get_broker`. Provider purposes now exist for a future migration without changing runtime code: `operator_system_control_read` is for `get_snapshot()` and `get_positions_summary()`, while `operator_system_control_emergency` is for `cancel_all_pending()` and `flatten_all()`. Emergency cancel/flatten operations are not read-only.
...
The next recommended provider step, if the migration continues, is a runtime-migration design for `SystemControlService` that preserves this tests/docs-only baseline and uses the scaffolded purpose split. `SystemControlService` remains direct/deferred. That future work should split read-only operator status from emergency write-capable operator actions before any runtime construction changes are made.
...
16. Before migrating any remaining direct write-capable or mixed path, add a tests-only/equivalence PR for exactly one chosen candidate. The PR should lock existing safety gates, credential source, direct/provider-unwired baseline, provider request purpose, fake broker boundary, and unchanged read/write behavior.
17. Only after the remaining direct construction paths are migrated where appropriate, design a second adapter spike using recorded/non-live fixtures. Do not add live trading or strategy-driven broker writes as part of that spike.

## Next Recommended PR

Keep `SystemControlService` deferred until a runtime migration can separate read-only operator status from emergency write-capable operator actions, or give those surfaces separate provider purposes and acceptance tests. Retain the tests for no-broker/decryption/environment skips, fake broker boundaries, and `ExecutionEngine` routing before changing runtime construction. `cancel_timed_out_orders` is now provider-backed but must remain classified as write-capable because it can cancel broker orders through `ExecutionEngine.cancel_order(...)`.
