...
`apps/api/tests/unit/test_position_monitor_provider_equivalence.py` adds focused equivalence coverage for the current `PositionMonitor` boundary without migrating runtime construction. It locks `_get_broker` as direct/provider-unwired, proves mock mode, no active connection, credential-decryption failure, policy rejection, active encrypted credential use, and construction order against fakes, and verifies `require_broker_environment(...)` still runs after credential decryption and before direct `Trading212Adapter` construction. It also proves `run(...)` kill-switch and auto-trading skips happen before broker lookup, proves account/position reads occur through fake brokers only, and proves automated exit and EOD flatten order-producing paths route intent and submission through a fake `ExecutionEngine` rather than direct broker write methods. No provider helper is called, and no real broker, network, cancellation, or order submission occurs in these tests.
...
| `apps/api/app/services/position_monitor.py` | `PositionMonitor._get_broker` | Local import and direct construction. | Monitors positions and submits automated exits or EOD flatten orders. | Mock returns mock adapter; active connection for `settings.APP_MODE`; decryption failure marks reconnect required; `require_broker_environment(conn.environment, action="position monitor broker access")`; higher-level app settings and strategy/risk checks in monitor flow. | Active encrypted `BrokerConnection` for current `settings.APP_MODE`. | Write-capable: reads positions/account data and can create and submit exit/flatten orders through the same broker helper. | Later. Not a safe first target because it is an automated exit engine and EOD flatten path. | Premature migration could expand automated exit write access or weaken position/environment checks. | Current provider-equivalence tests lock direct construction, no-broker/decryption/environment skips, account/position reads, fake-engine automated exit and EOD flatten submission, mock-mode dry-run flags, provider-unwired source evidence, and no direct broker writes. Before runtime migration, add or retain focused coverage for any additional live unlock, monitor-enabled, strategy/risk, and daily-loss gate combinations. |
