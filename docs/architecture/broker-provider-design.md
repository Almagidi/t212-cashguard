...
The provider migration now covers `get_broker()`, `/v1/broker/trading212` credential-test construction, scheduler startup construction, the terminal one-shot demo reconciliation worker, `sync_account_snapshot`, `track_cfd_funding`, `reconcile_pending_orders`, `cancel_timed_out_orders`, `PositionMonitor._get_broker`, `StrategyRunner._get_broker`, `PortfolioExecutionService._get_broker`, and `SystemControlService._get_broker`. The remaining direct `Trading212Adapter` construction/import inventory is locked by `apps/api/tests/unit/test_trading212_construction_inventory.py` and detailed in `docs/architecture/broker-interface-readiness-audit.md`.

Post-#80, that inventory classifies the only remaining direct references as `app/broker/trading212.py` itself, `app/broker/provider.py` as the canonical final-construction boundary, and the terminal-only/manual DEMO smoke scripts. If a production operational worker or service reintroduces direct `Trading212Adapter` import or construction, the construction inventory test must fail rather than accepting a broad wildcard. This tests/docs audit does not change runtime behaviour, enable live trading, add frontend controls, touch Dependabot/frontend dependency work, or add Kraken/crypto support.

`apps/api/tests/unit/test_write_capable_provider_boundary_audit.py` now adds the semantic lock for provider-backed write surfaces. It proves `cancel_timed_out_orders` is provider-backed with purpose `worker_cancel_timed_out_orders` and remains cancellation-capable, proves provider calls in `apps/api/app/workers/tasks.py` remain limited to `sync_account_snapshot`, `track_cfd_funding`, `reconcile_pending_orders`, and `cancel_timed_out_orders`, proves `PositionMonitor._get_broker` uses purpose `worker_position_monitor`, proves `StrategyRunner._get_broker` uses purpose `worker_strategy_runner`, proves `PortfolioExecutionService._get_broker` uses purpose `worker_portfolio_execution`, proves `SystemControlService._get_broker` is provider-backed, and classifies the service helpers as write-capable or mixed.

`sync_account_snapshot` now delegates only final Trading 212 adapter construction to `create_trading212_provider_adapter(...)`. It is still an isolated read-only worker task that reads account summary with `get_account_summary()`, uses active encrypted `BrokerConnection` credentials, and does not submit, cancel, or modify broker orders. Active connection lookup, credential decryption, reconnect-required handling, environment gates, provider-validation failure summarization, and snapshot persistence remain in worker code. Write-capable service helpers stay covered by focused provider-equivalence tests for their safety gates and broker write boundaries.
...
None of the remaining direct paths should be migrated from this document alone. Each needs focused tests-only/equivalence coverage before runtime construction changes. For already migrated write-capable or mixed paths including `cancel_timed_out_orders`, `position_monitor`, `strategy_runner`, `portfolio_execution_service`, and `system_control`, future edits must preserve dry-run/live behavior, kill switch, auto-trading and domain gates where applicable, active encrypted credential use, provider purposes `worker_cancel_timed_out_orders`, `worker_position_monitor`, `worker_strategy_runner`, `worker_portfolio_execution`, `operator_system_control_read`, and `operator_system_control_emergency`, fake broker boundaries, risk/allocation gates, and `ExecutionEngine` order routing behavior.
...
- The demo reconciliation scheduler uses only `T212_DEMO_API_KEY`/`T212_DEMO_API_SECRET`; generic and live variables cannot satisfy demo configuration.
...
`apps/api/tests/integration/test_scheduler_worker_provider_equivalence.py` documents the migrated scheduler and worker Trading 212 construction behaviour. The tests cover the background scheduler's demo-only startup gates, provider request data, demo-specific credential selection, refusal to use generic or live credentials, and proof that unsafe states fail before provider construction. They also document that `DemoReconciliationWorker` receives an already-constructed broker while the terminal worker script calls the provider with demo-specific credentials under demo-only gates.
...
15. Done: add tests/docs-only provider-equivalence coverage for `SystemControlService`, locking the direct/provider-unwired mixed boundary before runtime migration.
...
23. Done: add provider-purpose scaffolding for SystemControl. `operator_system_control_read` is user-scoped and read/status-only; `operator_system_control_emergency` is user-scoped and write-capable/emergency.
24. Done: migrate `SystemControlService._get_broker` final adapter construction to the provider with purpose `operator_system_control_read` for read/status methods and `operator_system_control_emergency` for emergency cancel/flatten methods, preserving mock/no-connection/decryption/policy behavior, reconnect-required handling, user scoping, and `ExecutionEngine` routing.
25. Before migrating another direct path, add a tests-only/equivalence PR for exactly one selected candidate. It must prove that path's current direct/provider-unwired baseline, safety gates, credential source, provider request purpose, fake broker boundary, and unchanged read/write behavior. At this point the remaining direct runtime paths are manual smoke scripts rather than production service helpers.
26. Consider broker-neutral route design only after the Trading 212 provider migration is complete and behavior-equivalent.
27. Design any second broker with recorded/non-live fixtures. Do not add live trading or strategy-driven broker writes as part of that spike.
...
SystemControlService is now provider-backed with `operator_system_control_read` for `get_snapshot()` and `get_positions_summary()`, and `operator_system_control_emergency` for `cancel_all_pending()` and `flatten_all()`. `cancel_timed_out_orders`, PositionMonitor, StrategyRunner, PortfolioExecutionService, and SystemControlService are no longer future provider wiring work; they are provider-backed and remain write-capable or mixed/write-capable where their `ExecutionEngine` paths can cancel or submit orders. Emergency system-control operations are not read-only. This remains DEMO hardening only: live trading is still disabled/not live-ready, and no Kraken or crypto work has been added.
