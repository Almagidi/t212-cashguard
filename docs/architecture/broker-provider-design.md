...
The provider migration now covers `get_broker()`, `/v1/broker/trading212` credential-test construction, scheduler startup construction, the terminal one-shot demo reconciliation worker, `sync_account_snapshot`, `track_cfd_funding`, `reconcile_pending_orders`, and `PositionMonitor._get_broker`. The remaining direct `Trading212Adapter` construction/import inventory is locked by `apps/api/tests/unit/test_trading212_construction_inventory.py` and detailed in `docs/architecture/broker-interface-readiness-audit.md`.

`apps/api/tests/unit/test_write_capable_provider_boundary_audit.py` now adds the semantic lock for the remaining paths. It proves `cancel_timed_out_orders` is still direct/provider-unwired and cancellation-capable, proves provider calls in `apps/api/app/workers/tasks.py` remain limited to `sync_account_snapshot`, `track_cfd_funding`, and `reconcile_pending_orders`, proves `PositionMonitor._get_broker` uses purpose `worker_position_monitor`, and classifies the remaining service helpers as write-capable or mixed before any further migration.
...
20. Before migrating another direct path, add a tests-only/equivalence PR for exactly one selected candidate. It must prove that path's current direct/provider-unwired baseline, safety gates, credential source, provider request purpose, fake broker boundary, and unchanged read/write behavior.
21. Consider broker-neutral route design only after the Trading 212 provider migration is complete and behavior-equivalent.
22. Design any second broker with recorded/non-live fixtures. Do not add live trading or strategy-driven broker writes as part of that spike.
...
Keep remaining write-capable paths deferred until each has focused acceptance coverage. PositionMonitor is no longer future provider wiring work; it is provider-backed with purpose `worker_position_monitor` and remains write-capable. The next roadmap item should be StrategyRunner provider migration prep: retain or deepen tests for no-broker/decryption/environment skips, venue/promotion/allocation/risk blockers, dry-run/live behavior, fake broker boundaries, and `ExecutionEngine` entry/exit routing before changing runtime construction. `cancel_timed_out_orders` should remain especially late because it can cancel broker orders through the execution engine; migrate it only when unchanged cancellation selection, environment gating, credentials, and broker-write behaviour are proven against fakes. For `system_control`, the next useful step before migration is a design or tests PR that separates read-only status from emergency-write provider purposes.
