...
`SystemControlService._get_broker` is now covered by tests/docs-only provider-equivalence tests and remains direct/provider-unwired. The coverage locks the current mock-mode path, app-mode and connection-environment safety gates, active encrypted `BrokerConnection` credential use, reconnect-required `commit=True` handling on decrypt failure, optional `broker_user_id` scoping, and direct `Trading212Adapter` construction after those gates. It proves read/status paths such as `get_snapshot()` and `get_positions_summary()` need only broker read methods and do not use `ExecutionEngine.cancel_order(...)`, flatten/order submission, raw broker writes, or provider request objects. It also proves emergency `cancel_all_pending()` and `flatten_all()` are write-capable through `ExecutionEngine.cancel_order(...)` and order intent/submission routing. Emergency operations are not read-only, and no runtime provider migration has happened.
...
23. Before migrating another direct path, add a tests-only/equivalence PR for exactly one selected candidate. It must prove that path's current direct/provider-unwired baseline, safety gates, credential source, provider request purpose, fake broker boundary, and unchanged read/write behavior.
24. Consider broker-neutral route design only after the Trading 212 provider migration is complete and behavior-equivalent.
25. Design any second broker with recorded/non-live fixtures. Do not add live trading or strategy-driven broker writes as part of that spike.
...
Keep `SystemControlService` deferred until a runtime provider migration can split read-only operator status from emergency operator actions, or give those surfaces separate provider purposes and acceptance tests. `cancel_timed_out_orders`, PositionMonitor, StrategyRunner, and PortfolioExecutionService are no longer future provider wiring work; they are provider-backed with purposes `worker_cancel_timed_out_orders`, `worker_position_monitor`, `worker_strategy_runner`, and `worker_portfolio_execution` and remain write-capable or mixed/write-capable. Emergency system-control operations are not read-only.
