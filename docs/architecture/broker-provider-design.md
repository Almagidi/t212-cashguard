...
21. Before migrating another direct path, add a tests-only/equivalence PR for exactly one selected candidate. It must prove that path's current direct/provider-unwired baseline, safety gates, credential source, provider request purpose, fake broker boundary, and unchanged read/write behavior.
22. Consider broker-neutral route design only after the Trading 212 provider migration is complete and behavior-equivalent.
23. Design any second broker with recorded/non-live fixtures. Do not add live trading or strategy-driven broker writes as part of that spike.
