...
The provider migration now covers `get_broker()`, `/v1/broker/trading212` credential-test construction, scheduler startup construction, the terminal one-shot demo reconciliation worker, `sync_account_snapshot`, `track_cfd_funding`, `reconcile_pending_orders`, `cancel_timed_out_orders`, `PositionMonitor._get_broker`, `StrategyRunner._get_broker`, and `PortfolioExecutionService._get_broker`. The remaining direct `Trading212Adapter` construction/import inventory is locked by `apps/api/tests/unit/test_trading212_construction_inventory.py` and detailed in `docs/architecture/broker-interface-readiness-audit.md`.
...
Keep remaining write-capable paths deferred until each has focused acceptance coverage. `cancel_timed_out_orders`, PositionMonitor, StrategyRunner, and PortfolioExecutionService are no longer future provider wiring work; they are provider-backed with purposes `worker_cancel_timed_out_orders`, `worker_position_monitor`, `worker_strategy_runner`, and `worker_portfolio_execution` and remain write-capable or mixed/write-capable. For `system_control`, the next useful step before migration is a design or tests PR that separates read-only status from emergency-write provider purposes.
