...
`PortfolioExecutionService._get_broker` is now covered by tests/docs-only provider-equivalence tests and remains direct/provider-unwired. The coverage locks the current mock-mode path, no-active-connection behavior, active encrypted `BrokerConnection` credential source, reconnect-required handling on decrypt failure, connection-environment policy rejection before adapter construction, and direct `Trading212Adapter` construction after lookup, decryption, and `require_broker_environment(...)`. It also proves early `run_all_enabled(...)` kill-switch, auto-trading, and live-unlocked skips happen before broker lookup, and proves the same helper backs account/position reads and rebalance order-producing behavior, with dry-run rebalance submission routed through fake `ExecutionEngine` calls only; no runtime provider migration has happened.

`StrategyRunner._get_broker` is now covered by tests/docs-only provider-equivalence tests and remains direct/provider-unwired. The coverage locks the current mock-mode path, no-active-connection behavior, active encrypted `BrokerConnection` credential source, reconnect-required handling on decrypt failure, connection-environment policy rejection before adapter construction, and direct `Trading212Adapter` construction after lookup, decryption, and `require_broker_environment(...)`. It also proves early `run_all_enabled(...)` kill-switch and auto-trading skips happen before broker lookup, proves account/position reads happen through the shared fake broker helper, preserves dry-run entry and exit behavior, and proves live entry/exit order-producing paths route through fake `ExecutionEngine` intent/submission calls only; no runtime provider migration has happened.

`PositionMonitor._get_broker` is now covered by tests/docs-only provider-equivalence tests and remains direct/provider-unwired. The coverage locks the current mock-mode path, no-active-connection behavior, active encrypted `BrokerConnection` credential source, reconnect-required handling on decrypt failure, connection-environment policy rejection before adapter construction, and direct `Trading212Adapter` construction after lookup, decryption, and `require_broker_environment(...)`. It also proves early `run(...)` kill-switch and auto-trading skips happen before broker lookup, proves account/position reads happen through the shared fake broker helper, preserves the current mock-mode dry-run flag for monitor-produced orders, and proves automated exit and EOD flatten order-producing paths route through fake `ExecutionEngine` intent/submission calls only; no runtime provider migration has happened.

The other remaining direct paths are intentionally deferred:

- `strategy_runner` is mixed/write-capable because it reads account/positions and can submit strategy entry and exit orders through `ExecutionEngine`; its current direct/provider-unwired behavior is locked by tests only.
- `portfolio_execution_service` is mixed/write-capable because it reads account/positions and can submit rebalance orders through `ExecutionEngine`; its current direct/provider-unwired behavior is locked by tests only.
...
None of these paths should be migrated from this document alone. Each needs focused tests-only/equivalence coverage before runtime construction changes. For `system_control`, a later runtime migration should probably split read-only status from emergency-write paths, or use separate provider purposes and tests for those surfaces, because a single broad helper currently serves both. For `portfolio_execution_service`, the current tests lock direct construction, early `run_all_enabled(...)` gates, account/position reads, dry-run order routing, and the live promotion gate. Any later migration must add or retain focused allocation and risk blocker coverage, then preserve active encrypted credential use, provider purpose, fake broker boundaries, and order submission through `ExecutionEngine`. For `strategy_runner`, the current tests lock direct construction, early kill-switch and auto-trading gates, account/position reads, dry-run entry/exit behavior, fake-engine entry/exit submission routing, and provider-unwired source evidence. Any later migration must add or retain focused coverage for strategy enabled filtering, venue gates, promotion gates, allocation rejection, and risk blockers before changing runtime construction. For `position_monitor`, the current tests lock direct construction, no-broker/decryption/environment skips, account/position reads, mock-mode dry-run flags, and fake-engine automated exit and EOD flatten submission. Any later migration must preserve dry-run/live behavior, kill switch, auto-trading and monitor gates, active encrypted credential use, provider purpose, fake broker boundaries, daily-loss/risk gates, and `ExecutionEngine` order submission behavior.
...
16. Done: add tests/docs-only provider-equivalence coverage for `PortfolioExecutionService`, locking the current direct/provider-unwired mixed/write-capable boundary without runtime migration.
...
18. Done: add tests/docs-only provider-equivalence coverage for `PositionMonitor`, locking the current direct/provider-unwired write-capable boundary without runtime migration.
19. Before migrating another direct path, add a tests-only/equivalence PR for exactly one selected candidate. It must prove the current direct/provider-unwired baseline, safety gates, credential source, provider request purpose, fake broker boundary, and unchanged read/write behavior.
20. Consider broker-neutral route design only after the Trading 212 provider migration is complete and behavior-equivalent.
21. Design any second broker with recorded/non-live fixtures. Do not add live trading or strategy-driven broker writes as part of that spike.
...
Keep write-capable paths deferred. The next PR should be tests-only/equivalence for exactly one remaining candidate and should not migrate runtime construction. `cancel_timed_out_orders` should remain especially late because it can cancel broker orders through the execution engine; migrate it only when unchanged cancellation selection, environment gating, credentials, and broker-write behaviour are proven against fakes. Continue to avoid runtime migrations for `position_monitor`, `strategy_runner`, and `portfolio_execution_service` until follow-ups use the newly locked behaviour to design precise provider purposes and migration acceptance matrices. The next useful tests-only PR is focused risk/allocation/live-gate coverage for either `portfolio_execution_service` or `strategy_runner`; for `position_monitor`, a later follow-up should deepen daily-loss, monitor-enabled, and live-gate coverage before any provider wiring. For `system_control`, the next useful step before migration is a design or tests PR that separates read-only status from emergency-write provider purposes.
