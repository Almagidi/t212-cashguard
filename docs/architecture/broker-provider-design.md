...
The provider migration now covers `get_broker()`, `/v1/broker/trading212` credential-test construction, scheduler startup construction, the terminal one-shot demo reconciliation worker, `sync_account_snapshot`, `track_cfd_funding`, `reconcile_pending_orders`, `PositionMonitor._get_broker`, `StrategyRunner._get_broker`, and `PortfolioExecutionService._get_broker`. The remaining direct `Trading212Adapter` construction/import inventory is locked by `apps/api/tests/unit/test_trading212_construction_inventory.py` and detailed in `docs/architecture/broker-interface-readiness-audit.md`.
...
None of the remaining direct paths should be migrated from this document alone. Each needs focused tests-only/equivalence coverage before runtime construction changes. For `system_control`, a later runtime migration should probably split read-only status from emergency-write paths, or use separate provider purposes and tests for those surfaces, because a single broad helper currently serves both. For `position_monitor`, `strategy_runner`, and `portfolio_execution_service`, future edits must preserve dry-run/live behavior, kill switch, auto-trading and domain gates, active encrypted credential use, provider purposes `worker_position_monitor`, `worker_strategy_runner`, and `worker_portfolio_execution`, fake broker boundaries, risk/allocation gates, and `ExecutionEngine` order submission behavior.
...
22. Before migrating another direct path, add a tests-only/equivalence PR for exactly one selected candidate. It must prove that path's current direct/provider-unwired baseline, safety gates, credential source, provider request purpose, fake broker boundary, and unchanged read/write behavior.
23. Consider broker-neutral route design only after the Trading 212 provider migration is complete and behavior-equivalent.
24. Design any second broker with recorded/non-live fixtures. Do not add live trading or strategy-driven broker writes as part of that spike.
...
Keep remaining write-capable paths deferred until each has focused acceptance coverage. PositionMonitor, StrategyRunner, and PortfolioExecutionService are no longer future provider wiring work; they are provider-backed with purposes `worker_position_monitor`, `worker_strategy_runner`, and `worker_portfolio_execution` and remain write-capable or mixed/write-capable. `cancel_timed_out_orders` should remain especially late because it can cancel broker orders through the execution engine; migrate it only when unchanged cancellation selection, environment gating, credentials, and broker-write behaviour are proven against fakes. For `system_control`, the next useful step before migration is a design or tests PR that separates read-only status from emergency-write provider purposes.
