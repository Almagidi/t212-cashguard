# Trading 212 DEMO Pending Order Follow-Up QA
...
feature/t212-demo-pending-order-follow-up

## Purpose

Document a read-only follow-up check for the Trading 212 DEMO order created during the controlled multi-order placement smoke.
...
No further placement should be attempted while this order remains pending.

Reconciliation should not be rerun yet because the order has not moved into Trading 212 history. The correct next action is another read-only pending-order check later.
...
- The disposable controlled-order API was stopped after the check.
